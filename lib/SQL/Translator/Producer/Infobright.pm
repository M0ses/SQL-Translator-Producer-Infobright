package SQL::Translator::Producer::Infobright;

=head1 NAME

SQL::Translator::Producer::Infobright - Infobright-specific producer for SQL::Translator
...
This library does not ALTER BRIGHTHOUSE tables, it drop`s them and creates 
them new. 
!!!! IT DOES NOT CARE ABOUT YOUR DATA, DATA WILL BE LOST
...
my $__already_recreated_bh_tables={};
...
sub __table_engine_bh  {
  my ($table_name,$options) = @_;
  my $regex = $options->{bh_table_engine_regex};
  my @exclude_maps = ();

  if ( ref($options->{bh_table_exclude_map}) eq 'ARRAY' ) {
    @exclude_maps = @{$options->{bh_table_exclude_map}};
  }
...
sub create_field
{
    ...
    #
        ...
        unless ($size[0] <= 65535 || $data_type =~ /^varchar/ ) {
            $data_type = 'text';
            @size      = ();
        }
    }
...
sub alter_create_constraint
{
    ...
    my $constraint = create_constraint(@_);

    return undef unless $constraint;
    debug("DEBUG013: $constraint");
    return join( ' ',
                 'ALTER TABLE',
                 $table_name,
                 'ADD',
...
sub alter_table
{
    ...
    debug("Alter table $table_name"); 
    my $out = '';
    if (
        ref($to_table->options) eq 'HASH' &&
        $to_table->options->{ENGINE} eq 'BRIGHTHOUSE'
    ) {
...
#use Data::Dump;
#
#  debug("DEBUG010: " .Data::Dump::dumpf(
#    $diff_hash->{alter_table},
#    sub {
#        my ($ctx,$obj) = @_;
#        print "DEBUG015: ".$ctx->class. "\n";
#        print "DEBUG016: ".$ctx->reftype. "\n";
#        return {object=>$obj}
#    }
#    )
#  );
  ...
  $table->options({
    'CHARACTER SET' => $options->{default_mysql_charset}
  }) if ($options->{default_mysql_charset});
  ...
  #
  #}
#  debug("DEBUG001: ".ref($diff_hash->{alter_field}));
#  foreach my $part ( @{ $diff_hash->{alter_field} } ) {
#    debug("DEBUG002: ".ref($part));
#    foreach my $part2 ( @{ $part } ) {
#        debug("DEBUG003: ".ref($part2));
#        if (ref($part2) eq 'SQL::Translator::Schema::Field') {
#            debug("DEBUG004: table: ".$part2->table." field: ".$part2->name);
#        }
#    }
#  }
#  foreach my $part ( @{ $diff_hash->{alter_table} } ) {
#    debug("DEBUG005: ".ref($part));
#    if (ref($part) eq 'SQL::Translator::Schema::Table') {
#            debug("DEBUG007: table: ".$part->name);
#    }
#  }
  my @stmts = map {
    if (@{ $diff_hash->{$_} || [] }) {
  ...
  my @alternate_stmts=();
  my @tmp_stmts;
#  debug("DEBUG008: \@stmts : ".Dumper(\@stmts));
  while ( my $stmt = shift(@stmts) ) {
    if ($stmt =~ '^ALTER TABLE' ) {
        push(@tmp_stmts,$stmt)
    } else {
